...
func (u *UsersRepo) Register(req *ap.UserCreateReq) (*ap.Void, error) {
	...
	_, err := u.db.Exec(query, id, req.Email, req.Password, req.Username, req.Phone)
...
func (u *UsersRepo) CheckEmail(req *ap.CheckEmailReq) (*ap.ById, error) {
	...
	if err != nil {
		log.Println("Error while getting user id: ", err)
		return nil, err
	}

	log.Println("Successfully got user id")
...
func (u *UsersRepo) GetByID(req *ap.ById) (*ap.UserRes, error) {
	query := "SELECT id, email, username FROM users WHERE id = $1 AND deleted_at = 0"
	user := &ap.UserRes{}
	err := u.db.QueryRow(query, req.Id).Scan(&user.Id, &user.Email, &user.Username)
	if err != nil {
		return nil, err
	}
	return user, nil
